export function langgraphConstitution(config) {
  return `# LangGraph Agent Constitution — ${config.projectName}
...
Write results to .agent/evidence/{STORY-ID}/code-audit.md.
  ...
  ☐ Every node has a single responsibility — max 50 lines
  ☐ Errors returned as state — return {"error": str(e)}, never raise
  ☐ All prompts read from files in prompts/ — not hardcoded strings
  ...
  ☐ Every edge condition has a test
